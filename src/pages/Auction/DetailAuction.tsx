...
import { CancelAuctionContract, cancelBidAuction, claimBidAuction, createBidAuction, fetchAuctionItems, fetchBidItemsById, fetchMyBidItems, getSoldNftStatus, updateBidAuction } from "../../services/contracts/auction";
...
const DetailUserAuction = () => {
  ...
  const handleCreateOrUpdateBid = async () => {
    try {
      ...
      if (biddingStatus === 0) {
        const createBidTx = await createBidAuction(currentItemId, amount);
        if (createBidTx) {
          setBiddingStatus(1)
        ...
        const updateBidTx = await updateBidAuction(currentItemId, amount);
        if (updateBidTx) {
  ...
  const handleDeleteBid = async () => {
    try {
      ...
      setLoading(true)
      const cancelBidTx = await cancelBidAuction(currentItemId);
      if (cancelBidTx) {
        setBiddingStatus(0)
        setCurrentBid(0)
  ...
  const handleClaimPrize = async () => {
    try {
      ...
      setLoading(true)
      const claimPrizeTx = await claimBidAuction(currentItemId);
      if (claimPrizeTx) {
  ...
  const handleClaimBid = async () => {
    try {
      ...
      setLoading(true)
      const claimBidTx = await claimBidAuction(currentItemId);
      if (claimBidTx) {
        toast.success('Successfully NFT claimed ')
        setSold(true)
      }
  ...
  const handleCancelClaimPrize = async () => {
    try {
      ...
      const tx = await CancelAuctionContract(currentItemId)
      if (tx) {
        setSold(true)
  ...
  const getWinnerInfo = (bidderLists: any, getAuctionInfo: any) => {
    if (storeData.wallet === 'connected') {
      ...
      const result: any = bidderLists.filter((item: any) => item.bidPrice.toNumber() === high)
      if (result && result[0]?.bidder?.toLowerCase() === storeData.address) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const getAuctionInfo: any = await fetchAuctionItems(nftInfoById.tokenId, nftInfoById.tokenAddress, nftInfoById.start_date)
        setAuctionInfo(getAuctionInfo)
        setCurrentItemId(getAuctionInfo?.itemId + 1)

        const get_biddingInfo = await fetchMyBidItems();
        ...
        const getBiddingLists = await fetchBidItemsById(getAuctionInfo.itemId + 1);
                        ...
                        return (
                          <ul className="py-2 px-4 w-full flex justify-between" key={idx} >
                            <li className={`basis-[50%] text-base
                              ${storeData?.address === item?.bidder ? `bg-[#323232]` : ``}
                            `}>
